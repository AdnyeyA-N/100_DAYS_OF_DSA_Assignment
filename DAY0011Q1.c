...
- Print the resultant matrix after addition,
  with each row on a new line and elements separated by spaces
...
Explanation:
    Each element of the result matrix is the sum of the corresponding elements
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            ...
            int sum = matrix1[i * n + j] + temp_val;
            printf("%d", sum);
